...
export function buildRoutinePdfInnerHtml(r, rows, textMain, bgCard, border, darkMode, textMuted, es, currentWeek) {
  ...
  /* Botón de impresión manual: fallback visible cuando window.onload→print() no se dispara (mobile). Oculto al imprimir. */
  parts.push(
    '<button type="button" class="itpdf-print-btn" onclick="window.print()" ' +
    'style="background:#2563EB;color:#fff;border:none;border-radius:10px;' +
    'padding:10px 18px;font-family:\'DM Sans\',system-ui,-apple-system,Segoe UI,Roboto,sans-serif;' +
    'font-size:14px;font-weight:700;cursor:pointer;margin-bottom:16px;">' +
    (isEs ? 'Descargar / Imprimir PDF' : 'Download / Print PDF') +
    '</button>'
  );
...
function buildPdfStylesheet() {
  return [
    ...
    '.itpdf-day-title{font-family:Barlow Condensed,DM Sans,sans-serif;font-size:24px;font-weight:800;letter-spacing:0.1em;color:#2563EB;margin:30px 0 14px;padding-top:16px;border-top:4px solid #2563EB}',
    ...
    '.itpdf-block-head-txt{font-size:13px;font-weight:800;letter-spacing:0.14em;color:#2563EB;text-transform:uppercase}',
    ...
    '@media print{@page{margin:8mm;size:A4}body{padding:0}.itpdf-print-btn{display:none}' +
    '.itpdf-card{break-inside:avoid;page-break-inside:avoid}' +
    '.itpdf-block-head{break-after:avoid;page-break-after:avoid}' +
    '.itpdf-day-title:not(:first-of-type){break-before:page;page-break-before:always}}',
