...
export async function shareSessionSummaryImageUi({ resumenSesion, msg, toast2 }) {
  try {
    ...
    // Semana — si no hay semana no se dibuja nada (nunca inventar un total).
    if (resumenSesion.semana != null) {
      ctx.fillStyle = "#8B9AB2";
      ctx.font = "700 32px Arial";
      const totalSemanas = resumenSesion.totalSemanas || 0;
      const semanaTxt = (msg("SEMANA", "WEEK"))+" "+resumenSesion.semana+(totalSemanas > 0 ? " / "+totalSemanas : "");
      ctx.fillText(semanaTxt, X, y);
      y += 100;
    }
